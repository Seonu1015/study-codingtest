...
public class WordChain {
    ...
    public int[] solution(int n, String[] words) {
        ...
        for (int i = 1; i < words.length; i++) {
            int num = i % n + 1;
            int count = i / n + 1;
            if (prev.charAt(prev.length() - 1) != words[i].charAt(0) || map.containsKey(words[i])) {
                answer[0] = num;
                answer[1] = count;
                break;
            } else {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(w.solution(3, arr1)));
        System.out.println(Arrays.toString(w.solution(5, arr2)));
        System.out.println(Arrays.toString(w.solution(2, arr3)));
    }
}
